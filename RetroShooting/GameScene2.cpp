...
void GameScene2::Init()
{
	...
	obm.AddObject(new MapLayer(L"bg2", 0, -0.5f, 540.0f));
	obm.AddObject(new MapLayer(L"Platform4", 0, 0.0f, 950.0f));
	...
	obm.AddObject(enemy = new DefensiveEnemy({ 4500, 800 }));
	enemyList.push_back(enemy);
	obm.AddObject(enemy = new FlashEnemy({ 3000, 800 }));
	enemyList.push_back(enemy);

	/*for (int i = 0; i < 10; ++i)
	{
		obm.AddObject(enemy = new RushEnemy({ float(700), 800 }));
		enemyList.push_back(enemy);
	}*/
...
void GameScene2::Update(float deltaTime)
{
	...
	if (Input::GetInstance().GetKeyState('R') == InputState::Down)
		obm.AddObject(new Fade(2));
